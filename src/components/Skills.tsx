      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {skillCategories.map((category, index) => (
            <div
              key={category.title}
              className="glass rounded-2xl p-8 hover:bg-white/[0.06] transition-all duration-500 group animate-scale-up hover:scale-105"
            ...
            >
              ...
              <ul className="space-y-3">
                {category.skills.map((skill, skillIndex) => (
                  <li
                    key={skill}
                    className="text-foreground/60 hover:text-accent transition-colors duration-200 animate-fade-up"
